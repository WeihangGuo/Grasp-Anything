...
from grasp_anything.utils.dataset_processing import grasp, image
from .grasp_data import GraspDatasetBase


class OCIDGraspDataset(GraspDatasetBase):
    """
    ...
    def __init__(self, file_path, ds_rotate=0, **kwargs):
        ...
        """
        super(OCIDGraspDataset, self).__init__(**kwargs)
    ...
    def _get_crop_attrs(self, idx):
        gtbbs = grasp.GraspRectangles.load_from_ocid_grasp_file(self.grasp_files[idx])
        center = gtbbs.center
    ...
    def get_rgb(self, idx, rot=0, zoom=1.0, normalise=True):
        rgb_file = self.grasp_files[idx].replace("Annotations", "rgb").replace("txt", "png")
